...
def get_merged_detections(base_model, finetuned_model, frame, device_str):
    ...
    base_results = base_model.predict(frame, conf=DETECTION_CONF_THRESHOLD, device=device_str, verbose=False)[0]
    ...
    finetuned_results = finetuned_model.predict(frame, conf=DETECTION_CONF_THRESHOLD, device=device_str, verbose=False)[0]
    for (x1, y1, x2, y2), cls_id, conf in zip(
...
def main(
    camera_source="http://192.168.4.97:8080/video",  # <-- update to your IP Webcam address + /video
